...
class ContainerDetail(APIView):
    parser_classes = (JSONParser, FormParser, MultiPartParser,)
    permission_classes = (permissions.IsAuthenticated, IsPIorAssistantofUserOrReadOnly, IsInGroupContanier, )
...
class ShelfAlternative(APIView):
    ...
    def post(self, request, ct_id, tw_id, sf_id, format=None):
        ...
        try:
            ...
            primary_group_researcher = GroupResearcher.objects.all() \
                .filter(user_id=user.pk) \
                .first()
    ...
    def post(self, request, format=None):
        try:
            ...
            if(ori_box_match and target_box_match):
                ...
                # if not auth_user.is_superuser:
                #     obj = {'user': auth_user, 'container': original_container}
                #     self.check_object_permissions(request, obj)  # check the permission
                #
                #     obj = {'user': auth_user, 'container': target_container}
                #     self.check_object_permissions(request, obj)  # check the permission
                        ...
                        if sample is not None:
                            ...
                            if hasattr(sample, key):
                                ...
                                return Response({'detail': 'sample ' + key + ' saved!'},
                                                status=status.HTTP_200_OK)
                            return Response({'detail': 'Something went wrong, sample attribute not found!'},
                                            status=status.HTTP_400_BAD_REQUEST)
                    else:
...
# 2 samples, switch positions within 1 box
        ...
        try:
            ...
            if first_box is None:
                return Response({'detail': 'first box does not exist!'},
                                status=status.HTTP_400_BAD_REQUEST)
            # box researcher
            first_box_owner = User()
            first_box_researcher = BoxResearcher.objects.all()\
                .filter(box_id=first_box.pk).first()
            if first_box_researcher is not None:
                first_box_owner = get_object_or_404(User, pk=first_box_researcher.researcher_id)
            # second container
            ...
            # box researcher
            second_box_owner = User()
            second_box_researcher = BoxResearcher.objects.all()\
                .filter(box_id=second_box.pk).first()
            if second_box_researcher is not None:
                second_box_owner = get_object_or_404(User, pk=second_box_researcher.researcher_id)
            if not settings.USE_CSAMPLE:
                # first sample
                first_sample = Sample.objects.all()\
                    .filter(box_id=first_box.pk)\
                    .filter(vposition__iexact=first_sample_vposition)\
                    .filter(hposition=int(first_sample_hposition))\
                    .filter(occupied__iexact=1).first()
                # second sample
                second_sample = Sample.objects.all()\
                    .filter(box_id=second_box.pk)\
                    .filter(vposition__iexact=second_sample_vposition)\
                    .filter(hposition=int(second_sample_hposition))\
                    .filter(occupied__iexact=1).first()
                ...
                if first_sample is not None:
                    if auth_user is not first_box_owner:
                        # get sample1 owner
                        sample_researcher = SampleResearcher.objects.all() \
                            .filter(sample_id=first_sample.pk).first()
                        if sample_researcher is not None:
                            sample_owner = get_object_or_404(User, pk=sample_researcher.researcher_id)
                            if sample_owner is not None:
                                obj = {'user': sample_owner, 'container': first_container}
                                self.check_object_permissions(request, obj)  # check the permission
                ...
                if second_sample is not None:
                    if auth_user is not second_box_owner:
                        # get sample1 owner
                        sample_researcher = SampleResearcher.objects.all() \
                            .filter(sample_id=second_sample.pk).first()
                        if sample_researcher is not None:
                            sample_owner = get_object_or_404(User, pk=sample_researcher.researcher_id)
                            if sample_owner is not None:
                                obj = {'user': sample_owner, 'container': second_container}
                                self.check_object_permissions(request, obj)  # check the permission
                ...
                # first sample
                first_sample = CSample.objects.all()\
                    .filter(box_id=first_box.pk)\
                    .filter(vposition__iexact=first_sample_vposition)\
                    .filter(hposition=int(first_sample_hposition))\
                    .filter(occupied__iexact=1).first()
                # second sample
                second_sample = CSample.objects.all()\
                    .filter(box_id=second_box.pk)\
                    .filter(vposition__iexact=second_sample_vposition)\
                    .filter(hposition=int(second_sample_hposition))\
                    .filter(occupied__iexact=1).first()
                ...
                if first_sample is not None:
                    if auth_user is not first_box_owner:
                        # get sample1 owner
                        sample_researcher = CSampleResearcher.objects.all() \
                            .filter(csample_id=first_sample.pk).first()
                        if sample_researcher is not None:
                            sample_owner = get_object_or_404(User, pk=sample_researcher.researcher_id)
                            if sample_owner is not None:
                                obj = {'user': sample_owner, 'container': first_container}
                ...
                if second_sample is not None:
                    if auth_user is not second_box_owner:
                        # get sample1 owner
                        sample_researcher = CSampleResearcher.objects.all() \
                            .filter(csample_id=second_sample.pk).first()
                        if sample_researcher is not None:
                            sample_owner = get_object_or_404(User, pk=sample_researcher.researcher_id)
                            if sample_owner is not None:
                                obj = {'user': sample_owner, 'container': second_container}
...
class DeleteAttachment(APIView):
    permission_classes = (permissions.IsAuthenticated, IsPIorAssistantorOwner, )
    ...
    def put(self, request, sp_id, at_id):
        try:
            ...
            if not settings.USE_CSAMPLE:
                sample = Sample.objects.all()\
                    .filter(id=int(sp_id)).first()
                if sample is None:
                    return Response({'detail': 'Sample not found!'},
                                    status=status.HTTP_400_BAD_REQUEST)
                attachment = SampleAttachment.objects.all()\
                    .filter(id=int(at_id)).first()
                ...
                if sample is None:
                    return Response({'detail': 'Sample not found!'},
                                    status=status.HTTP_400_BAD_REQUEST)
                attachment = CSampleAttachment.objects.all() \
                    .filter(id=int(at_id)).first()
                if attachment is None:
                    return Response({'detail': 'Attachment not found!'},
                                    status=status.HTTP_400_BAD_REQUEST)
    ...
    def post(self, request, sp_id):
        ...
        if not settings.USE_CSAMPLE:
            ...
            try:
                ...
                # load into dict
                obj = json.loads(model)
                serializer = UploadAttachmentSerializer(data=obj)
                serializer.is_valid(raise_exception=True)
                data = serializer.data
            ...
            sample_attachment = CSampleAttachment()
            try:
                auth_user = request.user
                user = request.user
                # parse data
                ...
                sample_attachment.label = data.get("label", "")
                sample_attachment.description = data.get("description", "")
                sample = get_object_or_404(CSample, pk=sp_id)
                if sample is not None:
                    sample_attachment.csample = sample
                    # save attachment
                    sample_attachment.attachment = form_data['file'][0]
                    # check permission
                    if user.is_superuser:
                    ...
                    # return the new attachment
                    # SampleAttachmentSerializer
                    serializer = CSampleAttachmentSerializer(sample_attachment)
                    return Response(serializer.data)
                return Response({'detail': 'sample not found!'},
                                status=status.HTTP_400_BAD_REQUEST)
            except:
    ...
    def post(self, request):
        try:
            if not settings.USE_CSAMPLE:
                ...
                csamples = CSample.objects.all().filter(id__in=sample_pks)
                serializer = CSampleSerializer(csamples, many=True)
                return Response(serializer.data)
        except:
